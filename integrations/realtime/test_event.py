...
def test__queue_event__persists_queued_event_to_database(state, session, db_event_1):
    ...
    with state.Session.begin() as s:
        row = s.scalar(select(QueuedEvent).where(QueuedEvent.id == queued.id))

    assert row is not None
    assert row.event == db_event_1.id
...
def test__pop_queued_event_bulk__leaves_unrelated_queued_events_intact(state, session, db_queued_event_1, db_queued_event_2):
    ...
    assert remaining[0].id == db_queued_event_2.id
...
def test__web_events_from_database__filters_by_encoded_event_name(state, session, db_event_1, db_event_2, mock_event_1):
    ...
    assert len(results) == 2
    assert all(isinstance(r, MockRealtimeEvent) for r in results)
...
def test__web_events_from_database__filters_by_namespace(state, session, db_event_1, db_event_2):
...
def test__queued_events_from_database__filters_by_encoded_event_name(
    state, session, mock_event_1, db_queued_event_1, db_queued_event_2
):
...
def test__queued_events_from_database__filters_by_event_id(state, session, db_event_1, db_queued_event_1, db_queued_event_2):
...
def test__queued_events_from_database__filters_by_namespace(state, session, db_queued_event_1, db_queued_event_2):
...
def test__queued_events_from_database__filters_by_after_datetime(state, session, db_queued_event_1, db_queued_event_2):
    ...
    results = EventStore().queued_events_from_database(
        state,
        encoded_event_names=[MockRealtimeEvent().__class__.__name__],
        after=future,
    )
