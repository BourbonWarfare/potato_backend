...
from bw.models.auth import Group, GroupPermission, UserGroup
...
class TestGroupStorePermissions:
    def test__create_permission__can_create_no_throw(self, state, permission_1):
    ...
    def test__create_permission__can_only_create_single_name(self, state, permission_1):
        GroupStore().create_permission(state, 'my permission', permission_1)
        with pytest.raises(GroupPermissionCreationFailed):
            GroupStore().create_permission(state, 'my permission', permission_1)

    def test__get_permission__can_get_valid_permission(self, state, db_permission_1):
    ...
    def test__get_permission__cant_get_invalid_permission(self, state, db_permission_1, permission_name_2):
        with pytest.raises(NoGroupPermissionWithCredentials):
            GroupStore().get_permission(state, permission_name_2)

    def test__edit_permission__can_edit_permission(self, state, db_permission_1, permission_2):
    ...
    def test__edit_permission__cant_edit_nonexistant_permission(self, state, db_permission_1, permission_2, permission_name_2):
        with pytest.raises(NoGroupPermissionWithCredentials):
    ...
    def test__get_all_permissions_user_has__user_has_expected_permissions_from_single_group(
        self, state, db_user_1, db_group_1, db_permission_1
    ):
    ...
    def test__get_all_permissions_user_has__user_has_expected_permissions_from_single_group_with_many(
        self, state, db_user_1, db_group_1, db_permission_1, db_group_2
    ):
        GroupStore().assign_user_to_group(state, db_user_1, db_group_1)
        permissions = GroupStore().get_all_permissions_user_has(state, db_user_1)
        assert db_permission_1.into_permissions().as_dict() == permissions.as_dict()
    ...
    def test__get_all_permissions_user_has__user_has_expected_permissions_from_many_groups(
        self, state, db_user_1, db_group_1, db_permission_1, db_group_2, db_permission_2
    ):
        GroupStore().assign_user_to_group(state, db_user_1, db_group_1)
    ...
    def test__get_all_permissions__empty_database_returns_empty_list(self, state, session):
        permissions = GroupStore().get_all_permissions(state)
        assert len(permissions) == 0

    def test__get_all_permissions__single_permission_returns_correctly(self, state, db_permission_1):
        permissions = GroupStore().get_all_permissions(state)
    ...
    def test__get_all_permissions__returns_correct_permission_details(
        self, state, db_permission_1, permission_1, permission_name_1
    ):
        permissions = GroupStore().get_all_permissions(state)
        perm = permissions[0]
    ...
    def test__get_all_permissions__permissions_are_detached_from_session(self, state, db_permission_1):
        permissions = GroupStore().get_all_permissions(state)
        assert permissions[0].name == db_permission_1.name
        assert permissions[0].id == db_permission_1.id

    def test__delete_permission__can_delete_existing_permission(self, state, db_permission_1):
        GroupStore().delete_permission(state, db_permission_1.name)
        with pytest.raises(NoGroupPermissionWithCredentials):
            GroupStore().get_permission(state, db_permission_1.name)

    def test__delete_permission__raises_on_nonexistent_permission(self, state, permission_name_1):
        with pytest.raises(NoGroupPermissionWithCredentials):
            GroupStore().delete_permission(state, permission_name_1)

    def test__delete_permission__can_delete_multiple_permissions(self, state, db_permission_1, db_permission_2, db_permission_3):
        GroupStore().delete_permission(state, db_permission_1.name)
    ...
    def test__delete_permission__delete_does_not_affect_other_permissions(self, state, db_permission_1, db_permission_2):
        GroupStore().delete_permission(state, db_permission_1.name)
    ...
    def test__delete_permission__empty_database_after_all_deleted(self, state, db_permission_1, db_permission_2, db_permission_3):
        GroupStore().delete_permission(state, db_permission_1.name)
        GroupStore().delete_permission(state, db_permission_2.name)
    ...
    def test__delete_permission__cannot_delete_same_permission_twice(self, state, db_permission_1):
        GroupStore().delete_permission(state, db_permission_1.name)
...
class TestGroupStoreGroups:
    def test__create_group__can_create_no_throw(self, state, db_permission_1):
    ...
    def test__create_group__can_only_create_single(self, state, db_permission_1):
        GroupStore().create_group(state, 'my group', db_permission_1.name)
        with pytest.raises(GroupCreationFailed):
            GroupStore().create_group(state, 'my group', db_permission_1.name)

    def test__create_group__cant_add_invalid_permission(self, state, session):
        with pytest.raises(GroupCreationFailed):
            GroupStore().create_group(state, 'my group', 'johnny')

    def test__delete_group__can_delete_empty(self, state, db_group_1):
        GroupStore().delete_group(state, db_group_1.name)

    def test__delete_group__can_delete_with_user(self, state, db_group_1, db_user_1):
        GroupStore().assign_user_to_group(state, db_user_1, db_group_1)
        GroupStore().delete_group(state, db_group_1.name)

    def test__delete_group__can_delete_with_users(self, state, db_group_1, db_user_1, db_user_2):
        GroupStore().assign_user_to_group(state, db_user_1, db_group_1)
        GroupStore().assign_user_to_group(state, db_user_2, db_group_1)
        GroupStore().delete_group(state, db_group_1.name)

    def test__delete_group__can_delete_with_user_only_one_group(self, state, db_group_1, db_user_1, db_group_2):
        GroupStore().assign_user_to_group(state, db_user_1, db_group_1)
        GroupStore().assign_user_to_group(state, db_user_1, db_group_2)
        GroupStore().delete_group(state, db_group_1.name)
    ...
    def test__delete_group__no_error_if_group_does_not_exist(self, state, group_name_1):
        GroupStore().delete_group(state, group_name_1)

    def test__get_group__can_get_existing_group(self, state, db_group_1):
        group = GroupStore().get_group(state, db_group_1.name)
        assert group.id == db_group_1.id
        assert group.name == db_group_1.name

    def test__get_group__raises_on_nonexistent_group(self, state, group_name_1):
        with pytest.raises(NoGroupWithName):
            GroupStore().get_group(state, group_name_1)

    def test__get_all_groups__empty_database_returns_empty_list(self, state, session):
        groups = GroupStore().get_all_groups(state)
        assert len(groups) == 0

    def test__get_all_groups__single_group_returns_correctly(self, state, db_group_1):
        groups = GroupStore().get_all_groups(state)
        assert len(groups) == 1
    ...
    def test__get_all_groups__returns_correct_group_details(self, state, db_group_1, group_name_1, db_permission_1):
        groups = GroupStore().get_all_groups(state)
        group = groups[0]
    ...
    def test__get_all_groups__groups_are_detached_from_session(self, state, db_group_1):
        groups = GroupStore().get_all_groups(state)
        assert groups[0].name == db_group_1.name
        assert groups[0].id == db_group_1.id
        assert groups[0].permissions == db_group_1.permissions

    def test__get_all_groups__groups_with_users_still_returned(self, state, db_group_1, db_user_1):
        GroupStore().assign_user_to_group(state, db_user_1, db_group_1)
...
class TestGroupStoreUserAssignment:
    def test__assign_user_to_group__can_assign(self, state, db_group_1, db_user_1):
        GroupStore().assign_user_to_group(state, db_user_1, db_group_1)
        with state.Session.begin() as session:
    ...
    def test__assign_user_to_group__can_assign_only_once(self, state, db_group_1, db_user_1):
        GroupStore().assign_user_to_group(state, db_user_1, db_group_1)
        with pytest.raises(GroupAssignmentFailed):
            GroupStore().assign_user_to_group(state, db_user_1, db_group_1)

    def test__assign_user_to_group__can_assign_many_users_to_group(self, state, db_group_1, db_user_1, db_user_2):
        GroupStore().assign_user_to_group(state, db_user_1, db_group_1)
        GroupStore().assign_user_to_group(state, db_user_2, db_group_1)
        with state.Session.begin() as session:
            query = select(UserGroup).where(UserGroup.user_id == db_user_1.id).where(UserGroup.group_id == db_group_1.id)
    ...
    def test__assign_user_to_group__can_assign_user_to_many_groups(self, state, db_group_1, db_user_1, db_group_2):
        GroupStore().assign_user_to_group(state, db_user_1, db_group_1)
        GroupStore().assign_user_to_group(state, db_user_1, db_group_2)
        with state.Session.begin() as session:
            query = select(UserGroup).where(UserGroup.user_id == db_user_1.id).where(UserGroup.group_id == db_group_1.id)
    ...
    def test__remove_user_from_group__can_remove_user(self, state, db_group_1, db_user_1):
        GroupStore().assign_user_to_group(state, db_user_1, db_group_1)
        with state.Session.begin() as session:
            query = select(UserGroup).where(UserGroup.user_id == db_user_1.id).where(UserGroup.group_id == db_group_1.id)
            row = session.execute(query).first()
    ...
    def test__remove_user_from_group__nothing_if_cant_remove_user(self, state, db_group_1, db_user_1):
        GroupStore().remove_user_from_group(state, db_user_1, db_group_1)

    def test__remove_user_from_group__no_error_if_user_not_in_group(self, state, db_group_1, db_user_2):
    ...
    def test__get_user_groups__returns_all_groups(self, state, db_user_1, db_group_1, db_group_2, db_group_3):
        GroupStore().assign_user_to_group(state, db_user_1, db_group_1)
        GroupStore().assign_user_to_group(state, db_user_1, db_group_2)
