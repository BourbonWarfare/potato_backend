...
class TestStartUnauthenticatedSession:
...
class TestExpireSessionFromUser:
    def test__expire_session_from_user__session_expires(self, mocker, token_1, expire_valid, state, db_user_1):
        mocker.patch('secrets.token_urlsafe', return_value=token_1)
        mocker.patch('bw.models.auth.Session.api_session_length', return_value=expire_valid)
...
class TestGetUserFromSessionToken:
    def test__get_user_from_session_token__correct_user_returned_from_token(self, state, db_session_1, db_session_2, db_user_1):
        assert SessionStore().get_user_from_session_token(state, db_session_1.token).id == db_user_1.id
...
class TestBourbonCode:
    def test__register_bourbon_code__code_created(self, session, state, token_1):
        email = 'test_register@example.com'
        SessionStore().register_bourbon_code(state, access_code=token_1, email=email)
    ...
    def test__verify_bourbon_code__email_returned_when_valid(self, session, state, db_bourbon_code_1):
        result_email = SessionStore().verify_bourbon_code(state, access_code=db_bourbon_code_1.code)

        assert result_email == db_bourbon_code_1.email

    def test__verify_bourbon_code__record_deleted_when_verified(self, session, state, db_bourbon_code_1):
        SessionStore().verify_bourbon_code(state, access_code=db_bourbon_code_1.code)
    ...
    def test__verify_bourbon_code__none_returned_when_invalid_access_code(self, session, state, db_bourbon_code_1):
        result = SessionStore().verify_bourbon_code(state, access_code='invalid_code_xyz')

        assert result is None

    def test__verify_bourbon_code__none_returned_when_expired(self, session, state, db_bourbon_code_expired):
