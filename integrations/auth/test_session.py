...
def test__session_store__is_session_valid__with_session(state, db_session_1, token_1):
...
def test__session_store__starting_session_activates(mocker, token_1, expire_valid, state, db_user_1):
    mocker.patch('secrets.token_urlsafe', return_value=token_1)
    mocker.patch('bw.models.auth.Session.api_session_length', return_value=expire_valid)
...
def test__session_store__expired_session_is_invalid(mocker, token_1, expire_invalid, state, db_user_1):
    mocker.patch('secrets.token_urlsafe', return_value=token_1)
...
def test__session_store__expiring_stops_session(mocker, token_1, expire_valid, state, db_user_1):
    mocker.patch('secrets.token_urlsafe', return_value=token_1)
    mocker.patch('bw.models.auth.Session.api_session_length', return_value=expire_valid)
...
def test__session_store__safe_to_expire_non_session(mocker, token_1, expire_valid, state, db_user_1):
    mocker.patch('secrets.token_urlsafe', return_value=token_1)
    mocker.patch('bw.models.auth.Session.api_session_length', return_value=expire_valid)
...
def test__session_store__many_sessions_only_latest_valid(mocker, token_1, token_2, expire_valid, state, db_user_1):
    mocker.patch('secrets.token_urlsafe', return_value=token_1)
    mocker.patch('bw.models.auth.Session.api_session_length', return_value=expire_valid)
...
def test__session_store__correct_user_returned_from_token(state, db_session_1, db_session_2, db_user_1):
...
def test__session_store__two_users_independent_sessions(mocker, token_1, token_2, expire_valid, state, db_user_1, db_user_2):
...
def test__session_store__expiring_one_user_does_not_affect_other(
    mocker, token_1, token_2, expire_valid, state, db_user_1, db_user_2
):
    mocker.patch('secrets.token_urlsafe', side_effect=[token_1, token_2])
    mocker.patch('bw.models.auth.Session.api_session_length', return_value=expire_valid)
...
def test__session_store__expired_session_replaced_with_new(mocker, token_1, expire_invalid, expire_valid, state, db_user_1):
    mocker.patch('secrets.token_urlsafe', return_value=token_1)
    mocker.patch('bw.models.auth.Session.api_session_length', return_value=expire_invalid)
...
def test__session_store__get_user_from_expired_token_raises(mocker, token_1, expire_invalid, state, db_user_1):
    mocker.patch('secrets.token_urlsafe', return_value=token_1)
    mocker.patch('bw.models.auth.Session.api_session_length', return_value=expire_invalid)
...
def test__session_store__register_oauth_code_stores_code(state, oauth_code_1, oauth_state_1):
...
def test__session_store__get_oauth_code_deletes_after_retrieval(state, db_oauth_code_2, oauth_state_2):
...
def test__session_store__get_oauth_code_raises_for_expired_code(state, db_oauth_code_expired, oauth_state_3):
    """Test that expired OAuth codes raise NoAccessCodeFound"""
    with pytest.raises(NoAccessCodeFound):
        SessionStore().get_discord_oauth_code(state, oauth_state_3)


def test__session_store__multiple_oauth_codes_independent(state, db_oauth_code_1, db_oauth_code_2, oauth_state_1, oauth_state_2):
...
def test__session_store__start_user_session_returns_correct_data(mocker, token_1, expire_valid, state, db_user_1):
...
def test__session_store__start_user_session_creates_active_session(mocker, token_1, expire_valid, state, db_user_1):
...
def test__session_store__start_user_session_raises_on_failure(mocker, state, db_user_1):
