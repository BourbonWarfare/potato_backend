...
def test__create_new_user_bot__can_create_and_get_token(mocker, state, token_1):
...
async def test__login_with_discord__can_login_when_user_exists(
    mocker, state, token_1, token_2, expire_valid, discord_id_1, db_discord_user_1
):
...
@pytest.mark.asyncio
async def test__login_with_discord__bad_token_fails(mocker, state, token_1, token_2, db_discord_user_1):
    class MockSessionObject:
        async def __aenter__(self):
            return self
...
def test__login_with_bot__can_login_when_user_exists(mocker, state, token_1, expire_valid):
    mocker.patch('secrets.token_urlsafe', return_value=token_1)
...
def test__login_with_bot__can_login_when_generic_expire(mocker, state, token_1, expire_valid):
    mocker.patch('secrets.token_urlsafe', return_value=token_1)
...
def test__login_with_bot__invalid_id(mocker, state, token_1):
    mocker.patch('secrets.token_urlsafe', return_value=token_1)
...
def test__is_session_active__with_session(state, db_session_1, token_1, db_user_1):
    assert AuthApi().is_session_active(state, token_1)


def test__is_session_active__with_expired_session(state, db_expired_session_1, token_1, db_user_1):
...
def test__does_user_have_roles__no_session(state, role_1):
    assert not AuthApi().does_user_have_roles(state, 'no-token', role_1)


def test__does_user_have_roles__invalid_session(mocker, state, role_1):
...
def test__does_user_have_roles__no_roles(state, db_session_1, token_1, role_1):
    assert not AuthApi().does_user_have_roles(state, token_1, role_1)


def test__does_user_have_roles__has_some_roles(state, db_user_1, db_session_1, token_1, role_1, role_2, role_name_1):
...
def test__does_user_have_roles__has_all_roles(state, db_user_1, db_session_1, token_1, role_1, role_name_1):
    UserStore().create_role(state, role_name_1, role_1)
    UserStore().assign_user_role(state, db_user_1, role_name_1)
    assert AuthApi().does_user_have_roles(state, token_1, role_1)


def test__does_user_have_permissions__no_session(state, permission_1):
    assert not AuthApi().does_user_have_permissions(state, 'no-token', permission_1)


def test__does_user_have_permissions__invalid_session(mocker, state, permission_1):
    mocker.patch('bw.auth.session.SessionStore.get_user_from_session_token', side_effect=SessionExpired)
    assert not AuthApi().does_user_have_permissions(state, 'some-token', permission_1)


def test__does_user_have_permissions__no_permissions(state, db_session_1, token_1, permission_1):
...
def test__does_user_have_permissions__has_some_permissions(
    state, db_user_1, db_session_1, token_1, db_group_1, permission_1, permission_2
):
...
def test__does_user_have_permissions__has_all_permissions(state, db_user_1, db_session_1, token_1, db_group_1, permission_1):
    GroupStore().assign_user_to_group(state, db_user_1, db_group_1)
    assert AuthApi().does_user_have_permissions(state, token_1, permission_1)


def test__does_user_have_permissions__group_has_no_permissions(state, db_user_1, db_session_1, token_1, db_group_3, permission_1):
    GroupStore().assign_user_to_group(state, db_user_1, db_group_3)
    assert not AuthApi().does_user_have_permissions(state, token_1, permission_1)


def test__revoke_discord_user_session__no_user(state, discord_id_1):
    response = AuthApi().revoke_discord_user_session(state, discord_id_1)
    assert response.status_code == 404


def test__revoke_bot_user_session__no_user(state, token_1):
    response = AuthApi().revoke_bot_user_session(state, token_1)
    assert response.status_code == 404


def test__revoke_discord_user_session__user_exists(state, db_discord_user_1, db_session_1):
...
def test__assign_role__no_role(state, db_user_1):
    response = AuthApi().assign_role(state, 'nonexistent_role', db_user_1.uuid)
    assert response.status_code == 404


def test__create_group_permission__success(state, permission_1, permission_name_1):
...
def test__auth_api__user_info_returns_correct_group_order(state, db_user_1, db_group_1, db_group_2):
    GroupStore().assign_user_to_group(state, db_user_1, db_group_1)
    GroupStore().assign_user_to_group(state, db_user_1, db_group_2)
...
def test__auth_api__user_info_different_users_different_data(state, db_user_1, db_user_2, db_group_1):
    GroupStore().assign_user_to_group(state, db_user_1, db_group_1)
...
def test__list_all_users__single_user_returns_correct_structure(state, db_user_1):
...
def test__get_all_roles__single_role_returns_correctly(state, db_role_1, role_name_1, role_1):
...
def test__get_all_roles__multiple_roles_returns_all(state, db_role_1, db_role_2, role_name_1, role_name_2, role_1, role_2):
...
def test__get_all_roles__role_permissions_included(state, db_role_1, role_1):
...
def test__get_all_permissions__single_permission_returns_correctly(state, db_permission_1, permission_name_1, permission_1):
...
def test__get_all_permissions__multiple_permissions_returns_all(state, db_permission_1, db_permission_2, db_permission_3):
...
def test__get_all_permissions__permission_grants_included(state, db_permission_1, permission_1):
...
def test__get_all_groups__single_group_returns_correctly(state, db_group_1, group_name_1):
...
def test__get_all_groups__multiple_groups_returns_all(state, db_group_1, db_group_2, db_group_3):
...
def test__get_all_groups__group_permission_id_included(state, db_group_1, db_permission_1):
...
def test__delete_role__success_returns_ok(state, db_role_1, role_name_1):
    """Test that deleting existing role returns OK"""
    response = AuthApi().delete_role(state, role_name_1)
    assert response.status_code == 200


def test__delete_role__actually_deletes_role(state, db_role_1, role_name_1):
...
def test__delete_role__removes_from_users(state, db_role_1, db_user_1, role_name_1):
...
def test__delete_role__does_not_affect_other_roles(state, db_role_1, db_role_2, role_name_1):
...
def test__delete_permission__success_returns_ok(state, db_permission_1, permission_name_1):
    """Test that deleting existing permission returns OK"""
    response = AuthApi().delete_permission(state, permission_name_1)
    assert response.status_code == 200


def test__delete_permission__actually_deletes_permission(state, db_permission_1, permission_name_1):
...
def test__delete_permission__does_not_affect_other_permissions(state, db_permission_1, db_permission_2, permission_name_1):
...
def test__delete_group__success_returns_ok(state, db_group_1, group_name_1):
    """Test that deleting existing group returns OK"""
    response = AuthApi().delete_group(state, group_name_1)
    assert response.status_code == 200


def test__delete_group__actually_deletes_group(state, db_group_1, group_name_1):
...
def test__delete_group__removes_user_associations(state, db_group_1, db_user_1, group_name_1):
...
def test__delete_group__does_not_affect_other_groups(state, db_group_1, db_group_2, group_name_1):
...
def test__edit_role__success_returns_json_with_name(state, db_role_1, role_name_1, role_2):
...
def test__edit_role__actually_updates_role(state, db_role_1, role_name_1, role_1, role_2):
...
def test__edit_role__nonexistent_role_returns_error(state, role_1):
    """Test that editing non-existent role returns 404"""
    response = AuthApi().edit_role(state, 'nonexistent_role', role_1)
    assert response.status_code == 404


def test__edit_role__keeps_same_id(state, db_role_1, role_name_1, role_2):
...
def test__edit_role__does_not_affect_other_roles(state, db_role_1, db_role_2, role_name_1, role_2):
...
def test__edit_permission__success_returns_json_with_name(state, db_permission_1, permission_name_1, permission_2):
...
def test__edit_permission__actually_updates_permission(state, db_permission_1, permission_name_1, permission_1, permission_2):
...
def test__edit_permission__nonexistent_permission_returns_error(state, permission_1):
...
def test__edit_permission__keeps_same_id(state, db_permission_1, permission_name_1, permission_2):
...
def test__edit_permission__does_not_affect_other_permissions(
    state, db_permission_1, db_permission_2, permission_name_1, permission_2
):
...
def test__edit_permission__groups_using_permission_get_updated_permissions(
    state, db_group_1, db_user_1, db_permission_1, permission_name_1, permission_1, permission_2
):
