...
from bw.models.missions import Iteration, Mission, MissionType
...
class TestMissionTypeStore:
    def test__create_mission_type__can_create_mission_type(self, state):
    ...
    def test__create_mission_type__cant_create_duplicate(self, state):
        MissionTypeStore().create_mission_type(state, 'mission', 5, 1)
        with pytest.raises(CouldNotCreateMissionType):
            MissionTypeStore().create_mission_type(state, 'mission', 5, 1)

    def test__update_mission_type__can_update_mission(self, state, db_mission_type_1, db_mission_type_2):
    ...
    def test__update_mission_type__updating_none_raises(self, state):
        with pytest.raises(NoMissionTypeWithName):
            MissionTypeStore().update_mission_type(state, name='')

    def test__delete_mission_type__can_delete(self, state, db_mission_type_1):
    ...
    def test__delete_mission_type__nothing_deleting_null(self, state, db_mission_type_1):
    ...
    def test__mission_type_from_name__can_retrieve(self, state, db_mission_type_1):
    ...
    def test__mission_type_from_name__retrieve_none_raises(self, state):
        with pytest.raises(NoMissionTypeWithName):
            MissionTypeStore().mission_type_from_name(state, name='')


class TestMissionStore:
    def test__create_mission__can_create(self, state, db_user_1, db_mission_type_1):
        uuid = uuid4()
    ...
    def test__add_iteration__first_iteration_is_one(self, state, db_mission_1):
    ...
    def test__add_iteration__increments_iteration(self, state, db_mission_1, db_iteration_1):
        iteration = MissionStore().add_iteration(
            state,
            mission=db_mission_1,
    ...
    def test__all_mission_iterations__returns_all(self, state, db_mission_1, db_iteration_1, db_iteration_2):
    ...
    def test__add_iteration__invalid_mission_raises(self, state):
    ...
    def test__add_iteration__can_add_more_than_two(self, state, db_iteration_1, db_iteration_2, db_mission_1):
        iteration = MissionStore().add_iteration(
            state,
            mission=db_mission_1,
    ...
    def test__get_missions_by_author_with_title__returns_missions_only_with_title(
        self, state, db_user_1, db_mission_1, db_mission_1_1, db_mission_1_2
    ):
    ...
    def test__mission_with_iteration__returns_mission(self, state, db_mission_1, db_iteration_1):
        mission = MissionStore().mission_with_iteration(state, db_iteration_1)
        assert mission.id == db_iteration_1.mission_id

    def test__mission_with_uuid__returns_mission(self, state, db_mission_1):
    ...
    def test__mission_with_uuid__unknown_uuid_raises(self, state):
        with pytest.raises(MissionDoesNotExist):
            MissionStore().mission_with_uuid(state, uuid.UUID(int=0))

    def test__mission_with_uuid__ignores_server(self, state, db_mission_1):
        # mission_with_uuid is the server-agnostic form: no server filter applies.
        mission = MissionStore().mission_with_uuid(state, db_mission_1.uuid)
        assert mission.server == db_mission_1.server

    def test__mission_with_uuid_in_server__returns_mission(self, state, db_mission_1):
        mission = MissionStore().mission_with_uuid_in_server(state, db_mission_1.uuid, db_mission_1.server)
        assert mission.id == db_mission_1.id

    def test__mission_with_uuid_in_server__wrong_server_raises(self, state, db_mission_1):
        with pytest.raises(MissionDoesNotExist):
            MissionStore().mission_with_uuid_in_server(state, db_mission_1.uuid, 'not-a-real-server')

    def test__mission_with_uuid_in_server__unknown_uuid_raises(self, state, db_mission_1):
        with pytest.raises(MissionDoesNotExist):
            MissionStore().mission_with_uuid_in_server(state, uuid.UUID(int=0), db_mission_1.server)

    def test__iteration_with_mission_and_name__happy_path_returns_iteration(self, state):
        pass

    def test__iteration_with_mission_and_name__no_mission_id_raises(self, state):
        pass

    def test__iteration_with_mission_and_name__no_filename_raises(self, state):
        pass


class TestMissionHistoryStore:
    def test__add_played_mission__object_created(self, state):
        pass
