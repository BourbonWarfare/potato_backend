...
class QueuedEvent(Base):
    ...
    event: Mapped[int | None] = mapped_column(
        ForeignKey(Event.id, name='linked_event_for_queued'), nullable=False, primary_key=True
    )
...
class PublishedEvent(Base):
    ...
    event: Mapped[int | None] = mapped_column(
        ForeignKey(Event.id, name='linked_event_for_published'), nullable=False, primary_key=True
    )
