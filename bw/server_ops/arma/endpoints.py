...
def define_arma(api: Blueprint, local: Blueprint, html: Blueprint):
    ...
    @require_session
    @require_user_role(Roles.can_manage_server)
    async def reload_servers(session_user: User) -> WebResponse:
        """
        ### Reload server configuration

        *Docstring generated by AI.*

        Reloads the server configuration from disk. This refreshes the SERVERS global
        dictionary with any changes made to the mod configuration files. Requires the
        `can_manage_server` role to execute. Useful for applying configuration updates
        ...
        **Returns:**
        - `WebResponse`:
          - **Success (200)**: Server configuration reloaded successfully
          - **Error (403)**: HTTP 403 response if user lacks required role
          - **Error (500)**: HTTP 500 response for server errors

        **Example:**
        ```
        POST /api/v1/server_ops/arma/servers/reload

        Success response (200):
        OK
        ```
        """
        logger.info(f'User {session_user.id} is reloading server configuration')
        server_config_path = ENVIRONMENT.server_config_directory()
        return ArmaApi().reload_server_configs(server_config_path)
    ...
    async def healthcheck_server(server: str) -> JsonResponse:
        ...
        *Docstring generated by AI.*
        ...
        Checks if an Arma server is currently running by pinging it. If it is down, this endpoint returns 500
        ...
        **Returns:**
        - `JsonResponse`: JSON response containing process status
          - **Success (200)**: HTTP 200 response if server can be pinged
          - **Error (500)**: HTTP 500 response if server cannot be found
        ...
        **Example:**
        ...
        """
        server = urllib.parse.unquote_plus(server)
        server_obj = ArmaApi().get_server_from_string(server)
        logger.info(f'Getting the status of {server}')
        return await ArmaApi().server_ping('localhost', server_obj.server_port() + 1)

    @api.get('/<string:server>/status')
    @url_endpoint
    async def server_status(server: str) -> JsonResponse:
        """
        ### Get server status

        *Docstring generated by AI.*

        Returns the status of the Arma server. Uses the Steam API to query information
        about the server, such as what map the current mission is on, the lobby status,
        and playercount.
        ...
        **Returns:**
        - `JsonResponse`: JSON response containing process status
          - **Success (200)**: `{'name': 'My Server', 'mission': 'Altis Life', 'state': 'LOBBY', 'map', 'Altis', 'players': 15}`
          - **Error (404)**: HTTP 404 response if server configuration not found
          - **Error (500)**: HTTP 500 response for other server errors

        **Example:**
        ```
        GET /api/v1/server_ops/arma/main_server/status
