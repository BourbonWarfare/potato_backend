...
from bw.server_ops.arma.mod import Kind, Mod
...
class ArmaApi:
    ...
    def deploy_keys(self, server_name: str) -> WebResponse:
        ...
        **Example:**
        ```python
        response = await arma_api.deploy_keys('main_server')
        # Success: Ok()
        # Error: WebResponse(status=404, reason='ServerConfigNotFound')
        ```
        """
    ...
    @define_async_api
    async def update_mods(self, server_name: str) -> JsonResponse:
        """
        ### Update mods for an Arma server

        *Docstring generated by AI.*

        Updates all mods for the specified server via SteamCMD workshop downloads. Stops all
        servers that use any of the mods being updated, downloads the latest versions from
        Steam Workshop, then redeploys mods and keys to all affected servers and restarts them.
        Only non-manual mods are updated automatically.

        **Args:**
        - `server_name` (`str`): The name of the server whose modlist should be updated as defined in the server configuration.
        ...
        **Example:**
        ```python
        response = await arma_api.update_mods('main_server')
        # Success: JsonResponse({
        #     'server_status': [
        #         {
        #             'message': 'Server updated',
        ...
        mods_to_update = server.modlist().mods

        affected_servers = [s for s in SERVER_MAP.values() if s.modlist().has_mods_from(server.modlist())]
        ...
        logger.info(f'Updating Arma mods for {server_name} via SteamCMD')
        await Chain(
            steam.login(
                GLOBAL_CONFIGURATION.require('steam_username').get(),
                GLOBAL_CONFIGURATION.require('steam_password').get(),
        ...
        response = [await self.server_pid_status(server.server_name()) for server in affected_servers]

        return JsonResponse({'server_status': [dataclasses.asdict(r) for r in response]})
