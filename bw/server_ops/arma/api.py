...
class ArmaApi:
    @define_api
    def create_event(self, state: State, tag: str, message: str, server: str) -> JsonResponse | WebResponse:
        ...
        event = ArmaEventStore().create_event(state, tag=tag, message=message, server=server)
    ...
    async def get_out_of_date_workshop_mods(self, state: State, mods: Collection[Mod]) -> JsonResponse:
        ...
        **Example:**
        ...
        logger.info(f'Found {len(steam_mods)} to check')
