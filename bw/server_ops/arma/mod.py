...
class SteamWorkshopDetails:
    ...
    @classmethod
    def from_steam_json(cls, json: dict[str, Any]) -> Self:
        """
        ### Create a SteamWorkshopDetails instance from JSON data returned by the Steam API
        ...
        **Example:**
        ```python
        details = SteamWorkshopDetails.from_steam_json({
...
async def fetch_mod_details_from_workshop(mods: Collection['Mod']) -> dict[WorkshopId, SteamWorkshopDetails]:
    ...
    **Example:**
    ...
    ```
    """
    logger.info(f"Fetching {len(mods)} mod(s) detail's from the workshop")
...
class Mod:
    ...
    def download_path(self) -> Path:
        """
        ### Get the full filesystem path to the mod download directory
        ...
        **Example:**
        ...
        mod_path = mod.download_path()
        # Path('/mods/@ace')
        # or
        # Path('/mods/steamapps/workshop/content/107410/<workshop id>')
        ```
        """
        if self.workshop_id is not None:
            return self.directory / 'steamapps' / 'workshop' / 'content' / '107410' / str(self.workshop_id)
        else:
