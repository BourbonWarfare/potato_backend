...
class Modlist:
    ...
    def has_mods_from(self, other: 'Modlist' | Iterable[Mod]) -> bool:
        """
...
class ModStore:
        ...
        **Example:**
        ...
        with state.Session.begin() as session:
            ...
            try:
                ...
                # Check if it's a unique constraint violation for workshop_id
                if 'workshop_id' in str(e.orig) and 'UNIQUE' in str(e.orig):
                    raise ModAlreadyExists(workshop_details.workshop_id) from e
                else:
                    raise ModCreationFailed(workshop_details.workshop_id) from e
    ...
    def update_mod(self, state: State, mod_data: SteamWorkshopDetails) -> DbMod:
        ...
        **Raises:**
        ...
        - `TypeError`: If mod_data is not SteamWorkshopDetails or Mod.
    ...
    def get_out_of_date_mods(self, state: State, mods_to_check: Iterable[SteamWorkshopDetails]) -> list[DbMod]:
        with state.Session.begin() as session:
            for mod in mods_to_check:
                ...
                out_of_date_mods = list(session.scalars(query).all())
                session.expunge_all()
