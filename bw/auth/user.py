...
class UserStore:
    ...
    def user_from_email(self, state: State, email: str) -> User:
        """
        ...
        **Returns:**
        - `User`: The user object associated with the given email.

        **Raises:**
        - `NoUserWithGivenCredentials`: If no user with the given email exists.

        **Example:**
        ```python
        user_store = UserStore()
        user = user_store.user_from_email(state, 'me@example.com')
        # User(id=1, uuid=UUID('...'))
        ```
        """
        with state.Session.begin() as session:
            query = select(User).join(BourbonUser, BourbonUser.user_id == User.id).where(BourbonUser.email == email)
            try:
                user = session.execute(query).one()[0]
            except NoResultFound:
                raise NoUserWithGivenCredentials(email)
            session.expunge(user)
        return user

    def user_from_username(self, state: State, username: str) -> User:
        """
        ...
        **Returns:**
        - `User`: The user object associated with the given username.

        **Raises:**
        - `NoUserWithGivenCredentials`: If no user with the given username exists.

        **Example:**
        ```python
        user_store = UserStore()
        user = user_store.user_from_username(state, 'tcvm')
        # User(id=1, uuid=UUID('...'))
        ```
        """
        with state.Session.begin() as session:
            query = select(User).join(BourbonUser, BourbonUser.user_id == User.id).where(BourbonUser.username == username)
            try:
                user = session.scalars(query).one()
            except NoResultFound:
                raise NoUserWithGivenCredentials(username)
            session.expunge(user)
        return user
    ...
    def link_bourbon_user(self, state: State, username: str, email: str, plaintext_password: str, user: User) -> BourbonUser:
        ...
        *Docstring generated by AI.*
        ...
        Links a Bourbon user to an existing user.

        **Async:** No

        **Args:**
        - `state` (`State`): The application state containing the database connection.
        - `email` (`str`): The email to associate with this user
        - `plaintext_password` (`str`): The password (in plaintext) which the user wil use
        - `user` (`User`): The user to link as a Bourbon user.

        **Returns:**
        - `DiscordUser`: The created Bourbon user object.

        **Raises:**
        - `NoUserWithGivenCredentials`: If the user does not exist.
        - `EmailAlreadyExists`: If the email is already linked to another user.
        - `DbError`: If there is an attempt to violate model constraints.

        **Example:**
        ```python
        user_store = UserStore()
        discord_user = user_store.link_bourbon_user(state, 'me@example.com', 'hunter2', user)
        # BourbonUser(id=1, user_id=1, email='me@example.com', password_hashed='...', salt='...', verified=False)
        ```
        """
        with state.Session.begin() as session:
    ...
    def bourbon_user_from_user(self, state: State, user: User) -> BourbonUser:
        """
        ### Get the Bourbon User from the given user object

        Returns the Bourbon User for the account

        **Async:** No

        **Args:**
        - `state` (`State`): The application state containing the database connection.
        - `user` (`User`): The user to find.

        **Returns:**
        - `BourbonUser`: The Bourbon user object.

        **Raises:**
        - `NoUserWithGivenCredentials`: If the user does not have a BW account linked.

        **Example:**
        ```python
        user_store = UserStore()
        bw_user = user_store.bourbon_user_from_user(state, user)
        ```
        """
        with state.Session.begin() as session:
    ...
    def delete_user(self, state: State, user: User):
        ...
        **Example:**
        ...
        """
        with state.Session.begin() as session:
            self.delete_bourbon_user(state, user)
            self.delete_discord_user(state, user)
            self.delete_bot_user(state, user)
            query = delete(User).where(User.id == user.id)
            session.execute(query)

    def delete_bourbon_user(self, state: State, user: BourbonUser | User):
        """
        ### Delete a Bourbon user record

        *Docstring generated by AI.*

        Deletes a Bourbon user record from the database.

        **Async:** No

        **Args:**
        - `state` (`State`): The application state containing the database connection.
        - `user` (`Bourbon | User`): The BourbonUser or User object to delete.

        **Raises:**
        - `AuthError`: If the arguments are not of the type BourbonUser or User.

        **Example:**
        ```python
        user_store = UserStore()
        user_store.delete_bourbon_user(state, discord_user)
        # Bourbon user record deleted
        ```
        """
        with state.Session.begin() as session:
            if isinstance(user, BourbonUser):
                query = delete(BourbonUser).where(BourbonUser.id == user.id)
            elif isinstance(user, User):
                query = delete(BourbonUser).where(BourbonUser.user_id == user.id)
            else:
    ...
    def set_bourbon_user_password_from_email(self, state: State, email: str, plaintext_password: str) -> BourbonUser:
        ...
        *Docstring generated by AI.*
        ...
        Updates the password for the Bourbon user with the provided email address and marks the
        account as verified. A new salt is generated so recovered accounts do not reuse password
        hashing material from the previous password.

        **Async:** No

        **Args:**
        - `state` (`State`): The application state containing the database connection.
        - `email` (`str`): The email address of the Bourbon user to update.
        - `plaintext_password` (`str`): The new plaintext password to hash and store.

        **Raises:**
        - `NoUserWithGivenCredentials`: If no Bourbon user exists for the email address.

        **Returns:**
        - `BourbonUser`: The updated Bourbon user object.

        **Example:**
        ```python
        bourbon_user = UserStore().set_bourbon_user_password_from_email(state, 'me@example.com', 'hunter2')
        # BourbonUser(email='me@example.com', verified=True)
        ```
        """
        salt = secrets.token_bytes(SALT_LENGTH)
        hashed_password = BourbonUser.hashed_password(plaintext_password, salt)
