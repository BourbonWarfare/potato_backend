...
class AuthApi:
    ...
    def send_recovery_email(self, state: State, email: str) -> WebResponse:
        ...
        Creates a recovery token and queues a recovery email when the provided email belongs to a
        Bourbon user. The response is always successful so callers cannot use the endpoint to
        ...
        **Args:**
        ...
        - `email` (`str`): The email address to send recovery instructions to.

        **Returns:**
        - `Created`: A Created response after the recovery request is accepted.

        **Example:**
        ```python
        response = AuthApi().send_recovery_email(state, 'me@example.com')
        # Success: Created(status=201)
        ```
        """
        logger.info('Accepting recovery email request')
        try:
            UserStore().user_from_email(state, email)
    ...
    @define_api
    def recover_bourbon_account(self, state: State, recovery_token: str, password: str) -> WebResponse:
        """
        ### Recover Bourbon account password

        Consumes a recovery token and sets a new password for the associated Bourbon user. Existing
        sessions for that user are expired after the password changes.

        *Docstring generated by AI.*

        **Args:**
        - `state` (`State`): The application state containing the database connection.
        - `recovery_token` (`str`): The recovery token sent to the user's email address.
        - `password` (`str`): The new password to store for the Bourbon user.

        **Returns:**
        - `Ok`: An OK response after the password is changed.

        **Example:**
        ```python
        response = AuthApi().recover_bourbon_account(state, 'token', 'hunter2')
        # Success: Ok(status=200)
        # Error: WebResponse(status=403, reason='invalid recovery token')
        ```
        """
        if not ALLOWED_PASSWORD_CHARACTERS.issuperset(password):
            return BadRequest('invalid characters in password')

        if not (email := SessionStore().verify_bourbon_recovery_code(state, recovery_token)):
            raise ForbiddenError('invalid recovery token')

        user = UserStore().user_from_email(state, email)
        UserStore().set_bourbon_user_password_from_email(state, email, password)
    ...
    def send_verification_email(self, state: State, email: str) -> WebResponse:
        ...
        Creates a verification token and queues a verification email for the provided email address.

        *Docstring generated by AI.*

        **Args:**
        - `state` (`State`): The application state containing the database connection.
        - `email` (`str`): The email address to send verification instructions to.

        **Returns:**
        - `Created`: A Created response after the verification request is accepted.

        **Example:**
        ```python
        response = AuthApi().send_verification_email(state, 'me@example.com')
        # Success: Created(status=201)
        ```
        """
    ...
    @define_api
    def resend_verification_email(self, state: State, email: str) -> WebResponse:
        """
        ### Resend verification email for an unverified user

        Queues a new verification email only when the provided email belongs to an unverified Bourbon
        user. The response is always successful so callers cannot determine whether an account exists.
        ...
        **Returns:**
        - `Created`: A Created response after the resend request is accepted.

        **Example:**
        ```python
        response = AuthApi().resend_verification_email(state, 'me@example.com')
        # Success: Created(status=201)
        ```
        """
        try:
        ...
        if bourbon_user.verified:
            return Created()

        return self.send_verification_email(state, email)
    ...
    @define_api
    def login_with_bourbon(self, state: State, username: str, password: str, redirect: str) -> WebResponse:
        """
        ### Log in with Bourbon account

        Logs in a user using their username and password, and starts a human session for them.
        Returns an error response if the user does not exist.
        ...
        **Args:**
        - `state` (`State`): The application state containing the database connection.
        - `username` (`str`): The users username (or email)
        - `password` (`str`): The users password
        - `redirect` (`str`): The URI which will be redirected to on return of this response

        **Returns:**
        - `WebResponse`: The result of the login

        **Example:**
        ```python
        response = AuthApi().login_with_bourbon(state, 'tcvm', 'hunter2')
        # Success: WebResponse(status=302)
        # Error: WebResponse(status=401, reason='Username/Password does not match')
        ```
        """
        try:
            user = UserStore().user_from_username(state, username)
        except NoUserWithGivenCredentials:
            user = UserStore().user_from_email(state, username)
    ...
    @define_api
    def is_session_authenticated(self, state: State, session_token: str) -> Exists:
        """
        ### Check if a session is authenticated

        Checks if a session with the given token is authenticated (logged in with an account).
        ...
        **Returns:**
        - `Exists`: An Exists response (200) if session is authenticated, DoesNotExist response (409) otherwise.

        **Example:**
        ```python
        response = AuthApi().is_session_authenticated(state, 'potato')
        # Success: Exists() (status 200, message 'Exists')
        # Not active: DoesNotExist() (status 409, message 'Does not exist')
        ```
        """
        return Exists(SessionStore().is_session_authenticated(state, session_token))
