...
from bw.models.auth import BourbonUserCode, DiscordOAuthCode, Session, User
...
class SessionStore:
    ...
    def start_unauthenticated_session(self, state: State) -> dict:
        """
        ### Start a new unauthenticated session

        Starts a new unauthenticated session. Not attached to any specific user.
        ...
        **Example:**
        ```python
        session_data = SessionStore().start_unauthenticated_session(state)
        ...
        with state.Session.begin() as session:
            query = (
                insert(Session)
                .values(token=token, expire_time=Session.unauthenticated_session_length(), authenticated=False)
    ...
    def is_session_authenticated(self, state: State, session_token: str) -> bool:
        """
        ### Check if a session is authenticated

        Checks if a session with the given token is currently authenticated (logged in with an account).
        ...
        **Example:**
        ```python
        is_active = SessionStore().is_session_authenticated(state, 'token123')
        # True (if session is valid and authenticated)
        # False (if session doesn't exist or is not authenticated)
        ```
        """
        with state.Session.begin() as session:
            # First check if session exists at all
    ...
    def register_bourbon_email_code(self, state: State, access_code: str, email: str):
        """
        ### Register Bourbon email access code

        Stores an email access code in the database to be queried later. The code has a short expiry
        time and can be used by verification or recovery flows.
        ...
        **Args:**
        - `state` (`State`): The application state containing the database connection.
        - `access_code` (`str`): The access code that authorizes the email action.
        - `email` (`str`): The email address associated with the access code.
        ...
        **Example:**
        ```python
        SessionStore().register_bourbon_email_code(state, 'code_123', 'me@example.com')
        # Code is stored in database with expiry time
        ```
        """
        with state.Session.begin() as session:
            model = BourbonUserCode(code=access_code, email=email)
            session.add(model)
            session.commit()

    def verify_bourbon_email_code(self, state: State, access_code: str) -> str | None:
        """
        ### Verify Bourbon email access code

        Retrieves and deletes an email access code if it exists and has not expired.
        ...
        **Args:**
        - `state` (`State`): The application state containing the database connection.
        - `access_code` (`str`): The access code created by an email flow.

        **Returns:**
        - `str | None`: The associated email when the code is valid, otherwise None.

        **Example:**
        ```python
        email = SessionStore().verify_bourbon_email_code(state, 'code_123')
        # 'me@example.com'
        # None if `code_123` is expired or does not exist
        ```
        """
        with state.Session.begin() as session:
            query = select(BourbonUserCode.expire_time, BourbonUserCode.email).where(BourbonUserCode.code == access_code)
            result = session.execute(query).tuples().one_or_none()

            if result is None:
                return None

            expire_time, email = result
            ...
            query = delete(BourbonUserCode).where(BourbonUserCode.code == access_code)
            session.execute(query)

            session.commit()
        return email

    def register_bourbon_code(self, state: State, access_code: str, email: str):
        self.register_bourbon_email_code(state, access_code, email)

    def verify_bourbon_code(self, state: State, access_code: str) -> str | None:
        return self.verify_bourbon_email_code(state, access_code)

    def register_bourbon_recovery_code(self, state: State, recovery_code: str, email: str):
        self.register_bourbon_email_code(state, recovery_code, email)

    def verify_bourbon_recovery_code(self, state: State, recovery_code: str) -> str | None:
        return self.verify_bourbon_email_code(state, recovery_code)
