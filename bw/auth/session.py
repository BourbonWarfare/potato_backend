...
class SessionStore:
    ...
    def register_bourbon_code(self, state: State, access_code: str, email: str):
        ...
        **Args:**
        - `state` (`State`): The application state containing the database connection.
        - `access_code` (`str`): The access code that will verify the user.
        ...
        **Example:**
        ```python
        SessionStore().register_bourbon_code(state, 'code_123')
        # Code is stored in database with expiry time
        ```
        """
        with state.Session.begin() as session:
    ...
    def verify_bourbon_code(self, state: State, access_code: str) -> str | None:
        """
        ### Verify if Bourbon code is valid

        Retrieves and deletes the access code associated with the given access code parameter from the
        registration flow. The code is only valid if it hasn't expired.
        ...
        **Args:**
        - `state` (`State`): The application state containing the database connection.
        - `access_code` (`str`): The access code parameter that the registration flow created.

        **Returns:**
        - `bool`: If the Bourbon code exists and is valid.

        **Example:**
        ```python
        code = SessionStore().get_bourbon_code(state, 'state_abc')
        # True
        # returns False if `state_abc` is expired or doesn't exist
        ```
        """
        with state.Session.begin() as session:
